...
cat('Some _summary statistics_ for the **R**\n`ChickWeight` data.')
...
--
.pull-right[
Some _summary statistics_ for the **R** `ChickWeight` data.

```{r cwSumary, echo=TRUE}
library(data.table)
cw <- as.data.table(ChickWeight)
cw[Time %in% c(0, 21)][ 
  , .(N = .N, Mean = mean(weight), 
      SD = sd(weight)), by = .(Diet, Time)][ 
    order(Diet, Time)] 
```
